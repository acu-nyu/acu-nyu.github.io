      ...
      <div className="relative aspect-[3/4] w-full overflow-hidden">
        {imageSrc ? (
          <img
            src={imageSrc}
            alt={name}
            className={`h-full w-full object-cover transition-transform duration-500 will-change-transform absolute inset-0 ${
              !isAlumni ? 'group-hover:scale-105' : ''
            }`}
            style={{ position: 'absolute', width: '100%', height: '100%' }}
          />
        ) : (
          <div className="h-full w-full bg-gray-800 flex items-center justify-center text-white/50 absolute inset-0">
            No Image
          </div>
        )}
